...
# DATE: 07-12-2025
# ASSIGNMENT: Max Increase to Keep City Skyline Analysis
...
# ─────────────────────────────── benchmark ──────────────────────────────
def benchmark(
    func: Callable, label: str, sizes: list[int], runs: int = 3
) -> pd.DataFrame:
    rec = []
    for n in sizes:
...
def main():
    ...
    for n in (1, 2):
        ...
        print(
            f"n={n}: greedy {t_g:.3f} ms | {m_g:.1f} KiB   "
